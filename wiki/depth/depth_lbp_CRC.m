...
for random_human = 0:1
    ...
    %            human_index = randperm(length(unique(ID_sub)));           % unique 去掉矩阵中重复的元素    % randperm 打乱序列
            ...
            tic
            label = L2_CRC(F_train, F_test, F_train_size, NumAct,0.0001);
            [confusion, accuracy, CR, FR] = confusion_matrix(label, F_test_size);
            ttt1 = toc;
        end
        strs = strcat('|random_human:',num2str(random_human),'|ratio参数：',num2str(ratio(ratio_i)),'|识别率:',num2str(mean(accuracy)),'|分类时间:',num2str(ttt1),'\n');
        fprintf(strs);
    end
end